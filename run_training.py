...
def run(dataset, data_dir, result_dir, config_id, num_gpus, total_kimg, gamma, mirror_augment, metrics):
    ...
    kwargs.update(dataset_args=dataset_args, sched_args=sched,
...
def main():
    ...
    parser.add_argument(
        '--num-gpus', help='Number of GPUs (default: %(default)s)', default=1, type=int, metavar='N')
    ...
    parser.add_argument('--metrics', help='Comma-separated list of metrics or "none" (default: %(default)s)',
                        default='fid50k', type=_parse_comma_sep)
